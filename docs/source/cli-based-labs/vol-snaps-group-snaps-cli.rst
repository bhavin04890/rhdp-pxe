...
.. code-block:: shell
  ...
  SNAP0=$(oc get volumesnapshotdatas.volumesnapshot.external-storage.k8s.io -n mysql -o jsonpath='{.items[0].metadata.name}')
  SNAP1=$(oc get volumesnapshotdatas.volumesnapshot.external-storage.k8s.io -n mysql -o jsonpath='{.items[1].metadata.name}')
  SNAP2=$(oc get volumesnapshotdatas.volumesnapshot.external-storage.k8s.io -n mysql -o jsonpath='{.items[2].metadata.name}')
